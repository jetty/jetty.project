          ...
          steps {
            container('jetty-build') {
              timeout( time: 240, unit: 'MINUTES' ) {
                mavenBuild( "jdk8u221", "clean install", "maven3")
    ...
    try {
      withEnv(["JAVA_HOME=${ tool "$jdk" }",
        ...
        configFileProvider(
          ...
          sh "mvn -fae -ntp -s $GLOBAL_MVN_SETTINGS -Dmaven.repo.local=.repository -Pci -V -B -e -Djetty.testtracker.log=true $cmdline -Dunix.socket.tmp=/tmp/unixsocket -Dmaven.test.failure.ignore=true"
        }
      }
    }
    finally
