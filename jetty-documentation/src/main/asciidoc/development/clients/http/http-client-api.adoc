...
The simple way to perform an HTTP request is the following:
...
The method `HttpClient.GET(...)` performs an HTTP `GET` request to the given URI and returns a `ContentResponse` when the request/response conversation completes successfully.
