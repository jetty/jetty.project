...
You can manually download the aggregate link:https://repo1.maven.org/maven2/org/eclipse/jetty/aggregate/jetty-tutorial/{VERSION}/jetty-tutorial-{VERSION}-uber.jar[`jetty-tutorial.jar`] using `curl` or a browser.
...
____
[IMPORTANT]
The `jetty-tutorial` jar referenced in this section is for example purposes only and should not be used outside of this context.
...
> mkdir Demo
> cd Demo
> curl -o jetty-tutorial-uber.jar https://repo1.maven.org/maven2/org/eclipse/jetty/aggregate/jetty-tutorial/{VERSION}/jetty-tutorial-{VERSION}-uber.jar
....
...
....
> mkdir classes
> javac -d classes -cp jetty-tutorial-uber.jar HelloWorld.java
....
...
[source, screen, subs="{sub-order}"]
....
> java -cp classes:jetty-tutorial-uber.jar org.eclipse.jetty.embedded.HelloWorld
....
