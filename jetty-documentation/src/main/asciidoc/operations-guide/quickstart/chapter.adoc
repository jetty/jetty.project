...
Also in previous versions of Jetty, it was necessary to use a context xml file and change the type from `org.eclipse.jetty.webapp.WebAppContext` to `org.eclipse.jetty.quickstart.QuickStartWebApp`.
This is will no longer work: just use the normal `org.eclipse.jetty.webapp.WebAppContext`.
