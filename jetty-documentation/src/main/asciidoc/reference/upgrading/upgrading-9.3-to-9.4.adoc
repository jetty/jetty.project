...
Applications using both `web.xml` and a `ServletContainerInitializer` (eg by implementing Spring's `WebApplicationInitializer`) must now declare a webapp version of at least 2.5. Previously, Jetty would load initializers even if the webapp version was older.
