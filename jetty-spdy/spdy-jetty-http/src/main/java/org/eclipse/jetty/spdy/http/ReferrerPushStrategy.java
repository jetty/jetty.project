 ...
 * <p>A typical request for a {@link MainResource} such as <tt>index.html</tt> is immediately
 ...
 * use to link the associated resource to the {@link MainResource}.</p>
 ...
 * has to be set. If the referrerPushPeriod for a {@link MainResource} has been passed, no more
 * associated resources will be added for that {@link MainResource}.</p>
 * <p>This class distinguishes associated {@link MainResource}s by their URL path suffix and content
 * type.
...
public class ReferrerPushStrategy implements PushStrategy
{
    ...
    private MainResource getOrCreateMainResource(String absoluteURL)
    {
        ...
        if (mainResource == null)
        {
            ...
            mainResource = new MainResource(absoluteURL);
            if(mainResources.putIfAbsent(absoluteURL, mainResource)!=null)
                mainResource = mainResources.get(absoluteURL);
        }
        return mainResource;
    }
