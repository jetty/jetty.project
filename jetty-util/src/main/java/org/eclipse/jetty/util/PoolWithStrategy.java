 ...
 *     <dt>{@link PoolWithStrategy.LeastRecentlyUsedStrategy}</dt>
 ...
 * @param <T>
 */
public class PoolWithStrategy<T> implements AutoCloseable, Dumpable
{
    private static final Logger LOGGER = Log.getLogger(PoolWithStrategy.class);
     ...
     */
    public PoolWithStrategy(int maxEntries, int cacheSize)
    {
     ...
     */
    public PoolWithStrategy(int maxEntries, Strategy<T> strategy)
    {
     ...
     * method called or be removed via {@link PoolWithStrategy.Entry#remove()} or
     * {@link PoolWithStrategy#remove(PoolWithStrategy.Entry)}.
     *
     ...
     * @return an entry from the pool or null if none is available.
     */
    public Entry acquire(Function<PoolWithStrategy<T>.Entry, T> creator)
    {
        Entry entry = acquire();
        if (entry != null)
            return entry;
     ...
     * false if the entry should be removed by calling {@link #remove(PoolWithStrategy.Entry)}
    ...
    public class Entry
    {
        ...
        /**
         * Release the entry.
         * This is equivalent to calling {@link PoolWithStrategy#release(PoolWithStrategy.Entry)} passing this entry.
         * @return true if released.
         */
        public boolean release()
        {
            return PoolWithStrategy.this.release(this);
        }

        /**
         * Remove the entry.
         * This is equivalent to calling {@link PoolWithStrategy#remove(PoolWithStrategy.Entry)} passing this entry.
         * @return true if remove.
         */
        public boolean remove()
        {
            return PoolWithStrategy.this.remove(this);
        }
    ...
    public interface Strategy<T>
    {
         ...
         */
        PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries);
         ...
         */
        default void released(PoolWithStrategy<T>.Entry entry)
        {
        }
    }
    ...
    public static class CompositeStrategy<T> implements Strategy<T>
    {
        ...
        @Override
        public PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries)
        {
            PoolWithStrategy<T>.Entry entry = planA.tryAcquire(entries);
            return entry != null ? entry : planB.tryAcquire(entries);
        }

        @Override
        public void released(PoolWithStrategy<T>.Entry entry)
        {
    ...
    public static class RetryStategy<T> implements Strategy<T>
    {
        ...
        public PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries)
        {
            ...
            while (tries-- > 0)
            {
                PoolWithStrategy<T>.Entry entry = strategy.tryAcquire(entries);
                if (entry != null)
                    return entry;
            }
            return null;
    ...
    public static class LinearSearchStrategy<T> implements Strategy<T>
    {
        ...
        public PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries)
        {
            for (PoolWithStrategy<T>.Entry e : entries)
            {
                if (e.tryAcquire())
                    return e;
            }
    ...
    public static class ThreadLocalIteratorStrategy<T> implements Strategy<T>
    {
        ...
        private final ThreadLocal<ListIterator<PoolWithStrategy<T>.Entry>> iterator = new ThreadLocal<>();
        ...
        public PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries)
        {
            ListIterator<PoolWithStrategy<T>.Entry> iter = iterator.get();
            if (iter == null)
            ...
            for (int tries = entries.size(); tries-- > 0; )
            {
                ...
                PoolWithStrategy<T>.Entry entry = iter.next();
    ...
    {
        private final ThreadLocal<PoolWithStrategy<T>.Entry> last;
        ...
        public PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries)
        {
            PoolWithStrategy<T>.Entry entry = last.get();
            if (entry != null && entry.tryAcquire())
                return entry;
            return null;
        }
    ...
    {
        private final ThreadLocal<List<PoolWithStrategy<T>.Entry>> cache;
        ...
        public PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries)
        {
            List<PoolWithStrategy<T>.Entry> cachedList = cache.get();
            while (!cachedList.isEmpty())
            {
                PoolWithStrategy<T>.Entry cachedEntry = cachedList.remove(cachedList.size() - 1);
    ...
    private abstract static class IndexedStrategy<T> implements Strategy<T>
    {
        ...
        public PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries)
        {
            ...
            try
            {
                PoolWithStrategy<T>.Entry entry = entries.get(i);
                if (entry != null && entry.tryAcquire())
                    return entry;
            }
            catch (IndexOutOfBoundsException e)
    ...
    public static class RandomIterationStrategy<T> implements Strategy<T>
    {
        ...
        public PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries)
        {
            ...
            ListIterator<PoolWithStrategy<T>.Entry> iter = entries.listIterator(r);
            while (iter.hasNext())
            {
                PoolWithStrategy<T>.Entry entry = iter.next();
                if (entry.tryAcquire())
    ...
    public static class RoundRobinIterationStrategy<T> implements Strategy<T>
    {
        ...
        public PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries)
        {
            ...
            ListIterator<PoolWithStrategy<T>.Entry> iter = entries.listIterator(i);
            while (iter.hasNext())
            {
                PoolWithStrategy<T>.Entry entry = iter.next();
                if (entry.tryAcquire())
    ...
    {
        Queue<PoolWithStrategy<T>.Entry> lru = new ConcurrentLinkedQueue<>();
        ...
        public PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries)
        {
            while (true)
            {
                PoolWithStrategy<T>.Entry entry = lru.poll();
                if (entry == null)
    ...
    public static class OneStrategyToRuleThemAll<T> implements Strategy<T>
    {
        ...
        public PoolWithStrategy<T>.Entry tryAcquire(List<PoolWithStrategy<T>.Entry> entries)
        {
            ...
            for (int tries = size; tries-- > 0;)
            {
                try
                {
                    PoolWithStrategy<T>.Entry entry = entries.get(index);
                    if (entry != null && entry.tryAcquire())
                        return entry;
                }
                catch (IndexOutOfBoundsException e)
