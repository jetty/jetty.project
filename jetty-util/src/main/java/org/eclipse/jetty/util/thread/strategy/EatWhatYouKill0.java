...
public class EatWhatYouKill0 extends ContainerLifeCycle implements ExecutionStrategy, Runnable
{
    private static final Logger LOG = Log.getLogger(EatWhatYouKill0.class);
    ...
    Mode EXECUTE_PRODUCE_CONSUME = new Mode("EXECUTE_PRODUCE_CONSUME")
    {
        boolean consume(Runnable task)
        {
            ...
            synchronized(EatWhatYouKill0.this)
            {
                switch (_state)
                {
                    case IDLE:
    ...
    public EatWhatYouKill0(Producer producer, Executor executor)
    {
        _producer = producer;
        _executor = executor;
