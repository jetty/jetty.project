 ...
 *
 * <p>
 * This strategy preemptively dispatches a thread as a pending producer, so that when a thread produces a task it can immediately run the task and let the
 * pending producer thread take over producing. If necessary another thread will be dispatched to replace the pending producing thread. When operating in this
 * pattern, the sub-strategy is called Execute Produce Consume (EPC)
 * </p>
 * <p>
 * However, if the task produced uses the {@link Invocable} API to indicate that it will not block, then the strategy will run it directly, regardless of the
 * presence of a pending producing thread and then resume producing after the task has completed. This sub-strategy is also used if the strategy has been
 * configured with a maximum of 0 pending threads and the thread currently producing does not use the {@link Invocable} API to indicate that it will not block.
 * When operating in this pattern, the sub-strategy is called ProduceConsume (PC).
 * </p>
 * <p>
 * If there is no pending producer thread available and if the task has not indicated it is non-blocking, then this strategy will dispatch the execution of the
 * task and immediately continue producing. When operating in this pattern, the sub-strategy is called ProduceExecuteConsume (PEC).
 * </p>
 */
@ManagedObject("eat what you kill execution strategy")
public class EatWhatYouKill3 extends ContainerLifeCycle implements ExecutionStrategy, Runnable
{
    private static final Logger LOG = Log.getLogger(EatWhatYouKill3.class);
    ...
    Mode EXECUTE_PRODUCE_CONSUME = new Mode("EXECUTE_PRODUCE_CONSUME")
    {
        boolean consume(Runnable task)
        {
            ...
            synchronized (EatWhatYouKill3.this)
            {
                switch (_state)
                {
                    case IDLE:
    ...
    Runnable nonBlockingProduce = new Runnable()
    {
        @Override
        public void run()
        {
    ...
    Runnable blockingProduce = new Runnable()
    {
        @Override
        public void run()
        {
    ...
    public EatWhatYouKill3(Producer producer, Executor executor)
    {
        _producer = producer;
        _executor = executor;
    ...
    private Runnable tryProduce()
    {
        switch (_state)
        {
            case IDLE:
                // Enter PRODUCING
                _state = State.PRODUCING;
                return Invocable.isNonBlockingInvocation()?nonBlockingProduce:blockingProduce;
