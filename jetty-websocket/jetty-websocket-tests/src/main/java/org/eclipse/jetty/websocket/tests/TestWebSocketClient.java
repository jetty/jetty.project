...
public class TestWebSocketClient extends TestWebSocket.Adapter implements TestWebSocket, AutoCloseable
{
    private final static Logger LOG = Log.getLogger(TestWebSocketClient.class);
    ...
    public TestWebSocketClient(LocalServer server) throws Exception
    {
        this(server, server.getWsUri(), null);
    }

    public TestWebSocketClient(LocalServer server, CharSequence requestPath) throws Exception
    {
        this(server, server.getWsUri().resolve(requestPath.toString()), null);
    }

    public TestWebSocketClient(LocalServer server, CharSequence requestPath, Map<String, String> upgradeRequestHeaders) throws Exception
    {
        this(server, server.getWsUri().resolve(requestPath.toString()), upgradeRequestHeaders);
    }

    public TestWebSocketClient(LocalServer server, URI wsUri, Map<String, String> upgradeRequestHeaders) throws Exception
    {
        super();
        this.server = server;
    ...
    public static class FrameCapture implements FrameHandler
    {
        ...
        public void onClosed(CloseStatus closeStatus)
        {
            ...
            // receivedFrames.offer(new Frame(OpCode.CLOSE).setPayload(closeStatus));
